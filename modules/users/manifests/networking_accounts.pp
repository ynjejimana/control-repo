class common::users::networking_accounts ($admin_group = hiera("common::users::admin_group::group_name"), $current_users, $former_users) {
	tag "autoupdate"

	require common::users::admin_group

	if defined(Class["common::users::ad_accounts"]) {
		fail("Cannot have both common::users::networking_accounts and common::users::ad_accounts on the same machine")
	}

	if defined(Class["common::ipa::client"]) {
		notify {"Skipping the addition of local accounts for networking in favour of IPA": }
	} else {
