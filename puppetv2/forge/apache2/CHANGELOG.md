...
- Allow setting KeepAlive related options per vhost
  - `apache2::vhost::keepalive`
  - `apache2::vhost::keepalive_timeout`
  - `apache2::vhost::max_keepalive_requests`
- Adds new class `apache2::mod::cluster`
...
- Added `apache2::balancer::target` attribute
- Added `apache2::fastcgi::server::pass_header` attribute
- Added ability for `apache2::fastcgi::server::host` using sockets
- Added `apache2::root_directory_options` attribute
- Added for `apache2::mod::ldap`:
  - `ldap_shared_cache_size`
  - `ldap_cache_entries`
  - `ldap_cache_ttl`
  - `ldap_opcache_entries`
  - `ldap_opcache_ttl`
- Added `apache2::mod::pagespeed::package_ensure` attribute
- Added `apache2::mod::passenger` attributes:
  - `passenger_log_level`
  - `manage_repo`
- Added upstream repo for `apache2::mod::passenger`
- Added `apache2::mod::proxy_fcgi` class
- Added `apache2::mod::security` attributes:
  - `audit_log_parts`
  - `secpcrematchlimit`
...
- Added `apache2::mod::ssl` attributes:
  - `ssl_mutex`
  - `apache_version`
- Added ubuntu 16.04 support
- Added `apache2::mod::authnz_ldap::package_name` attribute
- Added `apache2::mod::ldap::package_name` attribute
- Added `apache2::mod::proxy::package_name` attribute
- Added `apache2::vhost` attributes:
...
- Adds Configurability of Collaborative Detection Severity Levels for OWASP Core Rule Set to `apache2::mod::security` class
  - `critical_anomaly_score`
  - `error_anomaly_score`
  - `warning_anomaly_score`
  - `notice_anomaly_score`
- Adds ability to configure `info_path` in `apache2::mod::info` class
- Adds ability to configure `verify_config` in `apache2::vhost::custom`
...
- Fixed various `apache2::mod::*` file modes
- Fixed `apache2::mod::authnz_ldap` parameter `verifyServerCert` to be `verify_server_cert`
- Fixed loadfile name in `apache2::mod::fcgid`
- Fixed `apache2::mod::remoteip` to fail on apache < 2.4 (because it is not available)
- Fixed `apache2::mod::ssl::ssl_honorcipherorder` interpolation
- Lint fixes
- Strict variable fixes
- Fixed `apache2::vhost` attribute `redirectmatch_status` to be optional
...
- Added new 'apache2::vhost::custom' resource.
...
- Idempotency fixes with regards to '::apache2::mod_enable_dir'.
- ITK fixes.
...
#### Improvements
- apache2::mod::php now uses FilesMatch to configure the php handler. This is following the recommended upstream configuration guidelines (http://php.net/manual/en/install.unix.apache2.php#example-20) and distribution's default config (e.g.: http://bazaar.launchpad.net/~ubuntu-branches/ubuntu/vivid/php5/vivid/view/head:/debian/php5.conf). It avoids inadvertently exposing the PHP handler to executing uploads with names like 'file.php.jpg', but might impact setups with unusual requirements.
...
- change SSLProtocol in apache2::vhost to be space separated
- RewriteLock support

#### Bugfixes
- fix apache2::mod::cgid so it can be used with the event MPM 
...
- optionally set LimitRequestFieldSize on an apache2::vhost
...
#### Features
- Add `scan_proxy_header_field` parameter to `apache2::mod::geoip`
- Add `ssl_openssl_conf_cmd` parameter to `apache2::vhost` and `apache2::mod::ssl`
- Add `filters` parameter to `apache2::vhost`
...
#### Features
- Add `apache2::lib_path` parameter
- Add `apache2::service_restart` parameter
- Add `apache2::vhost::geoip_enable` parameter
- Add `apache2::mod::geoip` class
- Add `apache2::mod::remoteip` class
- Add parameters to `apache2::mod::expires` class
- Add `index_style_sheet` handling to `apache2::vhost::directories`
- Add some compatibility for SLES 11
- Add `apache2::mod::ssl::ssl_sessioncachetimeout` parameter
- Add `apache2::mod::ssl::ssl_cryptodevice` parameter
- Add `apache2::mod::ssl::ssl_honorcipherorder` parameter
- Add `apache2::mod::userdir::options` parameter

#### Bugfixes
- Document `apache2::user` parameter
- Document `apache2::group` parameter
- Fix apache2::dev on FreeBSD
- Fix proxy\_connect on apache >= 2.2
- Validate log levels better
- Fix `apache2::apache_name` for package and vhost
...
This release corrects a metadata issue that has been present since release 1.2.0. The refactoring of `apache2::vhost` to use `puppetlabs-concat` requires a version of concat newer than the version required in PE. If you are using PE 3.3.0 or earlier you will need to use version 1.1.1 or earlier of the `puppetlabs-apache` module.

## 2015-03-17 - Supported Release 1.4.0
###Summary

This release fixes the issue where the docroot was still managed even if the default vhosts were disabled and has many other features and bugfixes including improved support for 'deny' and 'require' as arrays in the 'directories' parameter under `apache2::vhost`

#### Features
- New parameters to `apache`
  - `default_charset`
  - `default_type`
- New parameters to `apache2::vhost`
  ...
  - `proxy_pass_match`
  - `no_proxy_uris_match`
- New parameters to `apache2::mod::passenger`
  - `passenger_app_env`
  - `passenger_min_instances`
- New parameter to `apache2::mod::alias`
  - `icons_options`
- New classes added under `apache2::mod::*`
  - `authn_file`
  - `authz_default`
  - `authz_user`
- Added support for 'deny' as an array in 'directories' under `apache2::vhost`
...
- Added proper array support for `require` in the `directories` parameter in `apache2::vhost`
- Added support for `setenv` inside proxy locations

### Bugfixes
- Fix issue in `apache2::vhost` that was preventing the scriptalias fragment from being included (MODULES-1784)
- Install required `mod_ldap` package for EL7 (MODULES-1779)
- Change default value of `maxrequestworkers` in `apache2::mod::event` to be a multiple of the default `ThreadsPerChild` of 25.
- Use the correct `mod_prefork` package name for trusty and jessie
- Don't manage docroot when default vhosts are disabled
- Ensure resources notify `Class['Apache2::Service']` instead of `Service['httpd']` (MODULES-1829)
...
- New parameters - `apache2::service`
  - `service_manage`
- New parameters - `apache2::vhost`
  - `access_logs`
  - `php_flags`
  - `php_values`
...
- Add `apache2::security::rule_link` define
- Improvements to `apache2::mod::*`
  - Add `apache2::mod::auth_cas` class
  - Add `threadlimit`, `listenbacklog`, `maxrequestworkers`, `maxconnectionsperchild` parameters to `apache2::mod::event`
  - Add `apache2::mod::filter` class
  - Add `root_group` to `apache2::mod::php`
  - Add `apache2::mod::proxy_connect` class
  - Add `apache2::mod::security` class
  - Add `ssl_pass_phrase_dialog` and `ssl_random_seed_bytes parameters to `apache2::mod::ssl` (MODULES-1719)
  - Add `status_path` parameter to `apache2::mod::status`
  - Add `apache_version` parameter to `apache2::mod::version`
  - Add `package_name` and `mod_path` parameters to `apache2::mod::wsgi` (MODULES-1458)
...
- Allow SSLCACertificatePath to be unset in `apache2::vhost` (MODULES-1457)
...
- `allow_encoded_slashes` was omitted from `apache2::vhost`
...
This release features many improvements and bugfixes, including several new defines, a reworking of apache2::vhost for more extensibility, and many new parameters for more customization. This release also includes improved support for strict variables and the future parser.

#### Features
- Convert apache2::vhost to use concat for easier extensions
- Test improvements
- Synchronize files with modulesync
- Strict variable and future parser support
- Added apache2::custom_config defined type to allow validation of configs before they are created
...
- Add support for reverse_urls/ProxyPassReverse in apache2::vhost
- Add satisfy directive in apache2::vhost::directories
- Add apache2::fastcgi::server defined type
- New parameters - apache
...
- Add support for ScriptAlias and ScriptAliasMatch in the apache2::vhost::aliases parameter
- Add support for rewrites in the apache2::vhost::directories parameter
- If the service_ensure parameter in apache2::service is set to anything other than true, false, running, or stopped, ensure will not be passed to the service resource, allowing for the service to not be managed by puppet
- Turn of SSLv3 by default
- Improvements to apache2::mod*
  - Add restrict_access parameter to apache2::mod::info
  - Add force_language_priority and language_priority parameters to apache2::mod::negotiation
  - Add threadlimit parameter to apache2::mod::worker
  - Add content, template, and source parameters to apache2::mod::php
  - Add mod_authz_svn support via the authz_svn_enabled parameter in apache2::mod::dav_svn
  - Add loadfile_name parameter to apache2::mod
  - Add apache2::mod::deflate class
  - Add options parameter to apache2::mod::fcgid
  - Add timeouts parameter to apache2::mod::reqtimeout
  - Add apache2::mod::shib
  - Add apache_version parameter to apache2::mod::ldap
  - Add magic_file parameter to apache2::mod::mime_magic
  - Add apache_version parameter to apache2::mod::pagespeed
  - Add passenger_default_ruby parameter to apache2::mod::passenger
  - Add content, template, and source parameters to apache2::mod::php
  - Add apache_version parameter to apache2::mod::proxy
  - Add loadfiles parameter to apache2::mod::proxy_html
  - Add ssl_protocol and package_name parameters to apache2::mod::ssl
  - Add apache_version parameter to apache2::mod::status
  - Add apache_version parameter to apache2::mod::userdir
  - Add apache2::mod::version class

#### Bugfixes
- Set osfamily defaults for wsgi_socket_prefix
- Support multiple balancermembers with the same url
- Validate apache2::vhost::custom_fragment
- Add support for itk with mod_php
- Allow apache2::vhost::ssl_certs_dir to not be set
...
- Use absolute scope to check class definition in apache2::mod::php
- Fix dependency loop in apache2::vhost
- Properly scope variables in the inline template in apache2::balancer
- Documentation clarification, typos, and formatting
- Set apache2::mod::ssl::ssl_mutex to default for debian on apache >= 2.4
...
- Fix misleading error message in apache2::version
...
- Added ssl_cipher parameter to apache2::mod::ssl
- Improved examples in documentation
- Added docroot_mode, action, and suexec_user_group parameters to apache2::vhost
...
- Fix bad dependencies in apache2::mod and apache2::mod::mime
...
- (docs) Update list of `apache2::mod::[name]` classes
- (docs) Fix apache2::namevirtualhost example call style
- Fix $ports_file reference in apache2::listen.
...
- Add new params to apache2::mod::mime class
- Allow apache2::mod to specify module id and path
...
- Remove `rewrite_base` `apache2::vhost` parameter; did not work anyway.
- Specify dependencies on stdlib >=2.4.0 (this was already the case, but
making explicit)
- Deprecate `a2mod` in favor of the `apache2::mod::*` classes and `apache2::mod`
defined resource.
...
- Add `apache2::mod::suphp` class for configuring SuPHP.
- Add `apache2::mod::itk` class for configuring ITK mpm module.
- Update `apache2::mod::wsgi` class for global WSGI configuration with
`wsgi_socket_prefix` and `wsgi_python_home` parameters.
- Add README.passenger.md to document the `apache2::mod::passenger` usage.
...
- Separate the httpd service resource into a new `apache2::service` class for
dependency chaining of `Class['apache'] -> <resource> ~>
Class['apache2::service']`
- Added `apache2::mod::proxy_balancer` class for `apache2::balancer`
...
#### Bugfixes:
- Update `apache2::mpm_module` detection for worker/prefork
- Update `apache2::mod::cgi` and `apache2::mod::cgid` detection for
worker/prefork

## 2013-07-16 Release 0.8.0
#### Features:
- Add `servername` parameter to `apache` class
- Add `proxy_set` parameter to `apache2::balancer` define

#### Bugfixes:
- Fix ordering for multiple `apache2::balancer` clusters
- Fix symlinking for sites-available on Debian-based OSs
- Fix dependency ordering for recursive confdir management
- Fix `apache2::mod::*` to notify the service on config change
- Documentation updates

## 2013-07-09 Release 0.7.0
#### Changes:
- Essentially rewrite the module -- too many to list
- `apache2::vhost` has many abilities -- see README.md for details
- `apache2::mod::*` classes provide httpd mod-loading capabilities
...
#### Changes:
- `include apache` is now required when using `apache2::mod::*`
...
* 57b9048 Commit apache2::vhost::redirect Manifest
* 8862d01 Commit apache2::vhost::proxy Manifest
* d5c1fd0 Commit apache2::mod::wsgi Manifest
* a825ac7 Commit apache2::mod::python Manifest
