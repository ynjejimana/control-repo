...
[`apache2::balancer`]: #defined-type-apachebalancer
[`apache2::balancermember`]: #defined-type-apachebalancermember
[`apache2::fastcgi::server`]: #defined-type-apachefastcgiserver
[`apache2::mod`]: #defined-type-apachemod
[`apache2::mod::<MODULE NAME>`]: #classes-apachemodmodule-name
[`apache2::mod::alias`]: #class-apachemodalias
[`apache2::mod::auth_cas`]: #class-apachemodauth_cas
[`apache2::mod::auth_mellon`]: #class-apachemodauth_mellon
[`apache2::mod::authnz_ldap`]: #class-apachemodauthnz_ldap
[`apache2::mod::cluster`]: #class-apachemodcluster
[`apache2::mod::disk_cache`]: #class-apachemoddisk_cache
[`apache2::mod::event`]: #class-apachemodevent
[`apache2::mod::ext_filter`]: #class-apachemodext_filter
[`apache2::mod::geoip`]: #class-apachemodgeoip
[`apache2::mod::itk`]: #class-apachemoditk
[`apache2::mod::ldap`]: #class-apachemodldap
[`apache2::mod::passenger`]: #class-apachemodpassenger
[`apache2::mod::peruser`]: #class-apachemodperuser
[`apache2::mod::prefork`]: #class-apachemodprefork
[`apache2::mod::proxy`]: #class-apachemodproxy
[`apache2::mod::proxy_fcgi`]: #class-apachemodproxy_fcgi
[`apache2::mod::proxy_html`]: #class-apachemodproxy_html
[`apache2::mod::security`]: #class-apachemodsecurity
[`apache2::mod::shib`]: #class-apachemodshib
[`apache2::mod::ssl`]: #class-apachemodssl
[`apache2::mod::status`]: #class-apachemodstatus
[`apache2::mod::worker`]: #class-apachemodworker
[`apache2::mod::wsgi`]: #class-apachemodwsgi
[`apache2::params`]: #class-apacheparams
[`apache2::version`]: #class-apacheversion
[`apache2::vhost`]: #defined-type-apachevhost
[`apache2::vhost::custom`]: #defined-type-apachevhostcustom
[`apache2::vhost::WSGIImportScript`]: #wsgiimportscript
...
``` puppet
apache2::vhost { 'cert.example.com':
  port     => '443',
...
To configure a mix of SSL and unencrypted virtual hosts at the same domain, declare them with separate [`apache2::vhost`][] defined types:

``` puppet
# The non-ssl virtual host
apache2::vhost { 'mix.example.com non-ssl':
...
apache2::vhost { 'mix.example.com ssl':
  servername => 'mix.example.com',
  port       => '443',
  docroot    => '/var/www/mix',
  ssl        => true,
...
To configure a virtual host to redirect unencrypted connections to SSL, declare them with separate [`apache2::vhost`][] defined types and redirect unencrypted requests to the virtual host with SSL enabled:

``` puppet
apache2::vhost { 'redirect.example.com non-ssl':
...
apache2::vhost { 'redirect.example.com ssl':
  servername => 'redirect.example.com',
  port       => '443',
  docroot    => '/var/www/redirect',
  ssl        => true,
...
``` puppet
apache2::vhost { 'aliases.example.com':
  serveraliases => [
    'aliases.example.org',
    'aliases.example.net',
  ],
...
``` puppet
apache2::vhost { 'suphp.example.com':
  port             => '80',
...
``` puppet
apache2::vhost { 'wsgi.example.com':
  port                        => '80',
...
``` puppet
apache2::vhost { 'wordpress.example.com':
  port             => '80',
...
``` puppet
apache2::vhost { 'rack.example.com':
  port           => '80',
...
``` puppet
apache2::vhost { 'The first IP-based virtual host, non-ssl':
...
apache2::vhost { 'The first IP-based vhost, ssl':
  servername => 'first.example.com',
  ip         => '10.0.0.10',
  port       => '443',
  ip_based   => true,
...
apache2::vhost { 'fifth.example.com':
  port       => '80',
...
- Use the [`apache2::mod::<MODULE NAME>`][] classes to [install specific Apache modules with parameters][Installing specific modules].
- Use the [`apache2::mod`][] defined type to [install arbitrary Apache modules][Installing arbitrary modules].

#### Installing specific modules

The Puppet apache module supports installing many common [Apache modules][], often with parameterized configuration options. For a list of supported Apache modules, see the [`apache2::mod::<MODULE NAME>`][] class references.

For example, you can install the `mod_ssl` Apache module with default settings by declaring the [`apache2::mod::ssl`][] class:

``` puppet
class { 'apache2::mod::ssl': }
```

[`apache2::mod::ssl`][] has several parameterized options that you can set when declaring it. For instance, to enable `mod_ssl` with compression enabled, set the [`ssl_compression`][] parameter to true:
...
Note that some modules have prerequisites, which are documented in their references under [`apache2::mod::<MODULE NAME>`][].

#### Installing arbitrary modules

You can pass the name of any module that your operating system's package manager can install to the [`apache2::mod`][] defined type to install it. Unlike the specific-module classes, the [`apache2::mod`][] defined type doesn't tailor the installation based on other installed modules or with specific parameters---Puppet only grabs and installs the module's package, leaving detailed configuration up to you.

For example, to install the [`mod_authnz_external`][] Apache module, declare the defined type with the 'mod_authnz_external' name:

``` puppet
apache2::mod { 'mod_authnz_external': }
```

There are several optional parameters you can specify when defining Apache modules this way. See the [defined type's reference][`apache2::mod`] for details.

### Configuring FastCGI servers to handle PHP files

Add the [`apache2::fastcgi::server`][] defined type to allow [FastCGI][] servers to handle requests for specific files. For example, the following defines a FastCGI server at 127.0.0.1 (localhost) on port 9000 to handle PHP requests:

``` puppet
apache2::fastcgi::server { 'php':
  host       => '127.0.0.1:9000',
  timeout    => 15,
  flush      => false,
...
Apache supports load balancing across groups of servers through the [`mod_proxy`][] Apache module. Puppet supports configuring Apache load balancing groups (also known as balancer clusters) through the [`apache2::balancer`][] and [`apache2::balancermember`][] defined types.

To enable load balancing with [exported resources][], export the [`apache2::balancermember`][] defined type from the load balancer member server:

``` puppet
@@apache2::balancermember { "${::fqdn}-puppet00":
...
Then declare the `apache2::balancer` and `apache2::balancermember` defined types on the proxy server.

If you need to use the [ProxySet](https://httpd.apache.org/docs/current/mod/mod_proxy.html#proxyset) directive on the balancer, use the [`proxy_set`](#proxy_set) parameter of `apache2::balancer`:

``` puppet
apache2::balancer { 'puppet01':
  proxy_set => {
...
- [**Public classes**](#public-classes)
    ...
    - [Class: apache2::dev](#class-apachedev)
    - [Classes: apache2::mod::\*](#classes-apachemodname)
- [**Private classes**](#private-classes)
    - [Class: apache2::confd::no_accf](#class-apacheconfdno_accf)
    - [Class: apache2::default_confd_files](#class-apachedefault_confd_files)
    - [Class: apache2::default_mods](#class-apachedefault_mods)
    - [Class: apache2::package](#class-apachepackage)
    - [Class: apache2::params](#class-apacheparams)
    - [Class: apache2::service](#class-apacheservice)
    - [Class: apache2::version](#class-apacheversion)
- [**Public defined types**](#public-defined-types)
    - [Defined type: apache2::balancer](#defined-type-apachebalancer)
    - [Defined type: apache2::balancermember](#defined-type-apachebalancermember)
    - [Defined type: apache2::custom_config](#defined-type-apachecustom_config)
    - [Defined type: apache2::fastcgi::server](#defined-type-fastcgi-server)
    - [Defined type: apache2::listen](#defined-type-apachelisten)
    - [Defined type: apache2::mod](#defined-type-apachemod)
    - [Defined type: apache2::namevirtualhost](#defined-type-apachenamevirtualhost)
    - [Defined type: apache2::vhost](#defined-type-apachevhost)
    - [Defined type: apache2::vhost::custom](#defined-type-apachevhostcustom)
- [**Private defined types**](#private-defined-types)
    - [Defined type: apache2::default_mods::load](#defined-type-default_mods-load)
    - [Defined type: apache2::peruser::multiplexer](#defined-type-apacheperusermultiplexer)
    - [Defined type: apache2::peruser::processor](#defined-type-apacheperuserprocessor)
    - [Defined type: apache2::security::file_link](#defined-type-apachesecurityfile_link)
- [**Templates**](#templates)
...
You can establish a default virtual host in this class, by using the [`apache2::vhost`][] defined type, or both. You can also configure additional specific virtual hosts with the [`apache2::vhost`][] defined type. Puppet recommends customizing the `apache` class's declaration with the following parameters, as its default settings are not optimized for production.
...
Configures module template behavior, package names, and default Apache modules by defining the version of Apache to use. Default: Determined by your operating system family and release via the [`apache2::version`][] class. Puppet recommends against manually configuring this parameter without reason.
...
- **Debian**: `/etc/apache/conf.d`
- **FreeBSD**: `/usr/local/etc/apache2`
- **Gentoo**: `/etc/apache/conf.d`
...
If this parameter's value is false, Puppet includes only the Apache modules required to make the HTTP daemon work on your operating system, and you can declare any other modules separately using the [`apache2::mod::<MODULE NAME>`][] class or [`apache2::mod`][] defined type.
...
- **FreeBSD**: `/usr/local/etc/apache2/server.crt`
...
- **FreeBSD**: `/usr/local/etc/apache2/server.key`
...
If true, Puppet automatically configures the following virtual host using the [`apache2::vhost`][] defined type:
...
By default, Puppet attempts to manage this group as a resource under the `apache` class, determining the group based on the operating system as detected by the [`apache2::params`][] class. To to prevent the group resource from being created and use a group created by another Puppet module, set the [`manage_group`][] parameter's value to false.
...
- **Debian**: `/etc/apache/mods-available`
- **FreeBSD**: `/usr/local/etc/apache2/Modules`
- **Gentoo**: `/etc/apache/modules.d`
- **Red Hat**: `/etc/httpd/conf.d`
...
- **Debian**: `/etc/apache/sites-available`
- **FreeBSD**: `/usr/local/etc/apache2/Vhosts`
- **Gentoo**: `/etc/apache/vhosts.d`
- **Red Hat**: `/etc/httpd/conf.d`
...
Default: Puppet sets the default value via the [`apache2::params`][] class, which manages the user based on your operating system:
...
The name of the Apache package to install. Default: Puppet sets the default value via the [`apache2::params`][] class, which manages the user based on your operating system:
...
Installs Apache development libraries. By default, the package name is defined by the [`dev_packages`][] parameter of the [`apache2::params`][] class based on your operating system:
...
> **Note**: On FreeBSD, you must declare the `apache2::package` or `apache` classes before declaring `apache2::dev`.

#### Classes: `apache2::mod::<MODULE NAME>`

Enables specific [Apache modules][]. You can enable and configure an Apache module by declaring its class. For example, to install and enable [`mod_alias`][] with no icons, you can declare the [`apache2::mod::alias`][] class with the `icons_options` parameter set to 'None':
...
The following Apache modules have supported classes, many of which allow for parameterized configuration. You can install other Apache modules with the [`apache2::mod`][] defined type.

* `actions`
* `alias` (see [`apache2::mod::alias`][])
* `auth_basic`
* `auth_cas`\* (see [`apache2::mod::auth_cas`][])
* `auth_mellon`\* (see [`apache2::mod::auth_mellon`][])
* `auth_kerb`
* `authn_core`
* `authn_file`
* `authnz_ldap`\* (see [`apache2::mod::authnz_ldap`][])
* `authz_default`
* `authz_user`
* `autoindex`
* `cache`
* `cgi`
* `cgid`
* `cluster` (see [`apache2::mod::cluster`][])
* `dav`
* `dav_fs`
* `dav_svn`\*
* `deflate\`
* `dev`
* `dir`\*
* `disk_cache` (see [`apache2::mod::disk_cache`][])
* `event` (see [`apache2::mod::event`][])
* `expires`
* `ext_filter` (see [`apache2::mod::ext_filter`][])
* `fastcgi`
* `fcgid`
* `filter`
* `geoip` (see [`apache2::mod::geoip`][])
* `headers`
* `include`
* `info`\*
* `itk`
* `ldap` (see [`apache2::mod::ldap`][])
* `mime`
* `mime_magic`\*
* `negotiation`
* `nss`\*
* `pagespeed` (see [`apache2::mod::pagespeed`][])
* `passenger`\* (see [`apache2::mod::passenger`][])
* `perl`
* `peruser`
* `php` (requires [`mpm_module`][] set to `prefork`)
* `prefork`\*
* `proxy`\* (see [`apache2::mod::proxy`][])
* `proxy_ajp`
* `proxy_balancer`
* `proxy_html` (see [`apache2::mod::proxy_html`][])
* `proxy_http`
* `python`
* `reqtimeout`
* `remoteip`\*
...
* `setenvif`
* `security`
* `shib`\* (see [`apache2::mod::shib`])
* `speling`
* `ssl`\* (see [`apache2::mod::ssl`][])
* `status`\* (see [`apache2::mod::status`][])
* `suphp`
* `userdir`\*
* `version`
* `vhost_alias`
* `worker`\*
* `wsgi` (see [`apache2::mod::wsgi`][])
* `xsendfile`
...
**Parameters within `apache2::mod::alias`**:
...
Installs and manages [`mod_mpm_event`][]. You can't include both `apache2::mod::event` and [`apache2::mod::itk`][], [`apache2::mod::peruser`][], [`apache2::mod::prefork`][], or [`apache2::mod::worker`][] on the same server.

**Parameters within `apache2::mod::event`**:
...
**Parameters within `apache2::mod::auth_cas`**:
...
**Parameters within `apache2::mod::auth_mellon`**:
...
**Parameters within `apache2::mod::authnz_ldap`**:
...
**Parameters within `apache2::mod::cluster`**:
...
**Parameters within `apache2::mod::deflate`**:
...
**Parameters within `apache2::mod::expires`**:
...
**Parameters within `apache2::mod::ext_filter`**:
...
If you include `apache2::mod::fcgid`, you can set the [`FcgidWrapper`][] per directory, per virtual host. The module must be loaded first; Puppet will not automatically enable it if you set the `fcgiwrapper` parameter in `apache2::vhost`.

``` puppet
include apache2::mod::fcgid

apache2::vhost { 'example.org':
  docroot     => '/var/www/html',
  directories => {
    path        => '/var/www/html',
    fcgiwrapper => {
...
**Parameters within `apache2::mod::geoip`**:
...
**Parameters within `apache2::mod::info`**:
...
- `apache_version`: Apache's version number as a string, such as '2.2' or '2.4'. Default: the value of [`$::apache2::apache_version`][`apache_version`].
...
**Parameters within `apache2::mod::passenger`**:
...
**Parameters within `apache2::mod::ldap`:**
...
##### Class: `apache2::mod::negotiation`

Installs and configures [`mod_negotiation`][].

**Parameters within `apache2::mod::negotiation`:**
...
**Parameters within `apache2::mod::pagespeed`**:
...
**Parameters within `apache2::mod::proxy`**:
...
**Parameters within `apache2::mod::php`**:
...
**Parameters within `apache2::mod::reqtimeout`**:
...
Defining this class enables Shibboleth-specific parameters in `apache2::vhost` instances.
...
**Parameters within `apache2::mod::ssl`**:
...
To use SSL with a virtual host, you must either set the [`default_ssl_vhost`][] parameter in `::apache` to true **or** the [`ssl`][] parameter in [`apache2::vhost`][] to true.

##### Class: `apache2::mod::status`

Installs [`mod_status`][] and uses the `status.conf.erb` template to generate its configuration.

**Parameters within `apache2::mod::status`**:
...
If Debian and Ubuntu systems with Apache 2.4 are classified with `apache2::mod::version`, Puppet warns that `mod_version` is built-in and can't be loaded.
...
**Parameters within `apache2::mod::security`**:

- `activated_rules`: An [array][] of rules from the `modsec_crs_path` to activate via symlinks. Default: `modsec_default_rules` in [`apache2::params`][].
...
- `crs_package`: Names the package that installs CRS rules. Default: `modsec_crs_package` in [`apache2::params`][].
- `modsec_dir`: Defines the path where Puppet installs the modsec configuration and activated rules links. Default: 'On', set by `modsec_dir` in [`apache2::params`][].
${modsec\_dir}/activated\_rules.
- `modsec_secruleengine`: Configures the modsec rules engine. Valid options: 'On', 'Off', and 'DetectionOnly'. Default: `modsec_secruleengine` in [`apache2::params`][].
...
**Parameters within `apache2::mod::wsgi`**:
...
- `wsgi_socket_prefix`: Defines the [`WSGISocketPrefix`][] directive, such as "\${APACHE\_RUN\_DIR}WSGI". Default: `wsgi_socket_prefix` in [`apache2::params`][].
...
#### Class: `apache2::version`

Attempts to automatically detect the Apache version based on the operating system.

### Public defined types

#### Defined type: `apache2::balancer`

Creates an Apache load balancing group, also known as a balancer cluster, using [`mod_proxy`][]. Each load balancing group needs one or more balancer members, which you can declare in Puppet with the  [`apache2::balancermember`][] define.

Declare one `apache2::balancer` define for each Apache load balancing group. You can export `apache2::balancermember` defined types for all balancer members and collect them on a single Apache load balancer server using [exported resources][].

**Parameters within `apache2::balancer`**:
...
If you statically declare all of your backend servers, set this parameter to false to rely on existing, declared balancer member resources. Also, use `apache2::balancermember` with [array][] arguments.
...
#### Defined type: `apache2::balancermember`
...
**Parameters within `apache2::balancermember`**:

##### `balancer_cluster`

**Required**. Sets the Apache service's instance name, and must match the name of a declared [`apache2::balancer`][] resource.
...
#### Defined type: `apache2::custom_config`
...
**Parameters within `apache2::custom_config`**:
...
Sets the directory in which Puppet places configuration files. Default: the value of [`$::apache2::confd_dir`][`confd_dir`].
...
#### Defined type: `apache2::fastcgi::server`
...
**Parameters within `apache2::fastcgi::server`:**
...
#### Defined type: `apache2::listen`

Adds [`Listen`][] directives to `ports.conf` in the Apache configuration directory that define the Apache server's or a virtual host's listening address and port. The [`apache2::vhost`][] class uses this defined type, and titles take the form '<PORT>', '<IPV4>:<PORT>', or '<IPV6>:<PORT>'.

#### Defined type: `apache2::mod`

Installs packages for an Apache module that doesn't have a corresponding [`apache2::mod::<MODULE NAME>`][] class, and checks for or places the module's default configuration files in the Apache server's `module` and `enable` directories. The default locations depend on your operating system.

**Parameters within `apache2::mod`**:
...
#### Defined type: `apache2::namevirtualhost`
...
The `apache2::vhost` defined type allows you to have specialized configurations for virtual hosts that have requirements outside the defaults. You can set up a default virtual host within the base `::apache` class, as well as set a customized virtual host as the default. Customized virtual hosts have a lower numeric [`priority`][] than the base class's, causing Apache to process the customized virtual host first.

The `apache2::vhost` defined type uses `concat::fragment` to build the configuration file. To inject custom fragments for pieces of the configuration that the defined type doesn't inherently support, add a custom fragment.

For the custom fragment's `order` parameter, the `apache2::vhost` defined type uses multiples of 10, so any `order` that isn't a multiple of 10 should work.

**Parameters within `apache2::vhost`**:
...
If [`apache2::mod::passenger`][] is loaded and `PassengerHighPerformance` is 'true', the `Alias` directive might not be able to honor the `PassengerEnabled => off` statement. See [this article](http://www.conandalton.net/2010/06/passengerenabled-off-not-working.html) for details.
...
Sets a given `apache2::vhost` defined type as the default to serve requests that do not match any other `apache2::vhost` defined types. Default: false.
...
``` puppet
apache2::vhost { 'sample.example.net':
  error_documents => [
...
Disables [`mod_security`][] on a virtual host. Only valid if [`apache2::mod::security`][] is included. Valid options: Boolean. Default: undef.
...
``` puppet
apache2::vhost { 'site.name.fdqn':
  …
  options => ['Indexes','FollowSymLinks','MultiViews'],
}
```

> **Note**: If you use the [`directories`][] parameter of [`apache2::vhost`][], 'Options', 'Override', and 'DirectoryIndex' are ignored because they are parameters within `directories`.
...
> **Note:** You should not need to use this parameter. However, if you do use it, be aware that the `default_vhost` parameter for `apache2::vhost` passes a priority of '15'.
...
``` puppet
apache2::vhost { 'setenv.example.com':
...
#### Parameter `directories` for `apache2::vhost`

The `directories` parameter within the `apache2::vhost` class passes an array of hashes to the virtual host to create [Directory](https://httpd.apache.org/docs/current/mod/core.html#directory), [File](https://httpd.apache.org/docs/current/mod/core.html#files), and [Location](https://httpd.apache.org/docs/current/mod/core.html#location) directive blocks. These blocks take the form, '< Directory /path/to/directory>...< /Directory>'.
...
``` puppet
apache2::vhost { 'files.example.net':
...
> **Note:** At least one directory should match the `docroot` parameter. After you start declaring directories, `apache2::vhost` assumes that all required Directory blocks will be declared. If not defined, a single default Directory block is created that matches the `docroot` parameter.
...
Note that you must declare `class { 'apache2::mod::ext_filter': }` before using this directive.

``` puppet
apache2::vhost { 'filter.example.org':
...
Note that you must declare `class {'apache2::mod::geoip': }` before using this directive.
...
Sets the [MellonEnable][`mod_auth_mellon`] directory to enable [`mod_auth_mellon`][]. You can use [`apache2::mod::auth_mellon`][] to install `mod_auth_mellon`.
...
Sets the value for the [PassengerEnabled](http://www.modrails.com/documentation/Users%20guide%20Apache.html#PassengerEnabled) directive to 'on' or 'off'. Requires `apache2::mod::passenger` to be included.
...
``` puppet
apache2::vhost{ 'filter.example.net':
  docroot     => '/path/to/directory',
  directories => [
    { path              => '/path/to/directory',
...
``` puppet
apache2::vhost { 'secure.example.net':
  docroot     => '/path/to/directory',
  directories => [
    { path        => '/path/to/directory',
...
> **Note**: If you include rewrites in your directories, also include `apache2::mod::rewrite` and consider setting the rewrites using the `rewrites` parameter in `apache2::vhost` rather than setting the rewrites in the virtual host's directories.

###### `shib_request_settings`

Allows a valid content setting to be set or altered for the application request. This command takes two parameters: the name of the content setting, and the value to set it to. Check the Shibboleth [content setting documentation](https://wiki.shibboleth.net/confluence/display/SHIB2/NativeSPContentSettings) for valid settings. This key is disabled if `apache2::mod::shib` is not defined. Check the [`mod_shib` documentation](https://wiki.shibboleth.net/confluence/display/SHIB2/NativeSPApacheConfig#NativeSPApacheConfig-Server/VirtualHostOptions) for more details.
...
When set to 'On', this turns on the use of request headers to publish attributes to applications. Valid options for this key is 'On' or 'Off', and the default value is 'Off'. This key is disabled if `apache2::mod::shib` is not defined. Check the [`mod_shib` documentation](https://wiki.shibboleth.net/confluence/display/SHIB2/NativeSPApacheConfig#NativeSPApacheConfig-Server/VirtualHostOptions) for more details.
...
#### SSL parameters for `apache2::vhost`
...
Specifies the SSL certification. Defaults are based on your OS: '/etc/pki/tls/certs/localhost.crt' for RedHat, '/etc/ssl/certs/ssl-cert-snakeoil.pem' for Debian, '/usr/local/etc/apache2/server.crt' for FreeBSD, and '/etc/ssl/apache2/server.crt' on Gentoo.
...
#### Defined type: `apache2::vhost::custom`

The `apache2::vhost::custom` defined type is a thin wrapper around the `apache2::custom_config` defined type, and simply overrides some of its default settings specific to the virtual host directory in Apache.

**Parameters within `apache2::vhost::custom`**:
...
#### Defined type: `apache2::peruser::multiplexer`

This defined type checks if an Apache module has a class. If it does, it includes that class. If it does not, it passes the module name to the [`apache2::mod`][] defined type.

#### Defined type: `apache2::peruser::multiplexer`

Enables the [`Peruser`][] module for FreeBSD only.

#### Defined type: `apache2::peruser::processor`

Enables the [`Peruser`][] module for FreeBSD only.

#### Defined type: `apache2::security::file_link`

Links the `activated_rules` from [`apache2::mod::security`][] to the respective CRS rules on disk.

### Templates

The Apache module relies heavily on templates to enable the [`apache2::vhost`][] and [`apache2::mod`][] defined types. These templates are built based on [Facter][] facts specific to your operating system. Unless explicitly called out, most templates are not meant for configuration.

## Limitations

### Ubuntu 10.04

The [`apache2::vhost::WSGIImportScript`][] parameter creates a statement inside the virtual host that is unsupported on older versions of Apache, causing it to fail. This will be remedied in a future refactoring.

### RHEL/CentOS 5

The [`apache2::mod::passenger`][] and [`apache2::mod::proxy_html`][] classes are untested since repositories are missing compatible packages.

### RHEL/CentOS 6

The [`apache2::mod::passenger`][] class is not installing as the the EL6 repository is missing compatible packages.

### RHEL/CentOS 7

The [`apache2::mod::passenger`][] and [`apache2::mod::proxy_html`][] classes are untested as the EL7 repository is missing compatible packages, which also blocks us from testing the [`apache2::vhost`][] defined type's [`rack_base_uris`][] parameter.
...
The [`apache2::vhost::wsgi_import_script`][] parameter creates a statement inside the virtual host that is unsupported on older versions of Apache, causing it to fail. This will be remedied in a future refactoring.

### RHEL/CentOS
The [`apache2::mod::auth_cas`][], [`apache2::mod::passenger`][], [`apache2::mod::proxy_html`][] and [`apache2::mod::shib`][] classes are not functional on RH/CentOS without providing dependency packages from extra repositories.
