...
describe 'apache2::fastcgi::server', :type => :define do
  ...
  describe 'os-dependent items' do
    context "on RedHat based systems" do
      ...
      it { should contain_class("apache2::mod::fastcgi") }
    ...
    context "on Debian based systems" do
      ...
      it { should contain_file("fastcgi-pool-#{title}.conf").with(
        :ensure => 'present',
        :path   => "/etc/apache/conf.d/fastcgi-pool-#{title}.conf"
      ) }
    end
    context "on FreeBSD systems" do
      ...
      it { should contain_file("fastcgi-pool-#{title}.conf").with(
        :ensure => 'present',
        :path   => "/usr/local/etc/apache4/Includes/fastcgi-pool-#{title}.conf"
      ) }
    end
