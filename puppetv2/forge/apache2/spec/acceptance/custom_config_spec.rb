...
describe 'apache2::custom_config define' do
  context 'invalid config' do
    it 'should not add the config' do
      pp = <<-EOS
        class { 'apache': }
        apache2::custom_config { 'acceptance_test':
          content => 'INVALID',
        }
      EOS
  ...
  context 'with a custom filename' do
    it 'should store content in the described file' do
      pp = <<-EOS
        class { 'apache': }
        apache2::custom_config { 'filename_test':
  ...
  describe 'custom_config without priority prefix' do
    it 'applies cleanly' do
      pp = <<-EOS
        class { 'apache': }
        apache2::custom_config { 'prefix_test':
          priority => false,
          content => '# just a comment',
        }
      EOS
  ...
  describe 'custom_config only applied after configs are written' do
    it 'applies in the right order' do
      pp = <<-EOS
        ...
        apache2::custom_config { 'ordering_test':
          content => '# just a comment',
        }

        # Try to wedge the apache2::custom_config call between when httpd.conf is written and
        # ports.conf is written. This should trigger a dependency cycle
        File["#{$conf_file}"] -> Apache2::Custom_config['ordering_test'] -> Concat["#{$ports_file}"]
      EOS
