...
describe 'apache2::vhost define' do
  ...
  context 'new proxy vhost on port 80' do
    it 'should configure an apache proxy vhost' do
      pp = <<-EOS
        class { 'apache': }
        apache2::vhost { 'proxy.example.com':
          port    => '80',
  ...
  when 'precise', 'wheezy'
    context 'vhost FallbackResource example' do
      it 'should configure a vhost with FallbackResource' do
        pp = <<-EOS
        class { 'apache': }
        apache2::vhost { 'fallback.example.net':
  ...
  context 'virtual_docroot hosting separate sites' do
    it 'should configure a vhost with VirtualDocumentRoot' do
      pp = <<-EOS
        class { 'apache': }
        apache2::vhost { 'virt.example.com':
          vhost_name      => '*',
  ...
  describe 'add_listen' do
    it 'applies cleanly' do
      pp = <<-EOS
        ...
        apache2::listen { '81': }
        apache2::vhost { 'testlisten.server':
          docroot    => '/tmp',
          port       => '80',
          add_listen => false,
  ...
  describe 'actions' do
    it 'applies cleanly' do
      ...
      EOS
      pp = pp + "\nclass { 'apache2::mod::actions': }" if fact('osfamily') == 'Debian'
      apply_manifest(pp, :catch_failures => true)
    end
