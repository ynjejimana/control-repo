...
describe 'apache2::mod::fcgid class', :if => ((fact('osfamily') == 'RedHat' and fact('operatingsystemmajrelease') != '5') and !(fact('operatingsystem') == 'OracleLinux' and fact('operatingsystemmajrelease') == '7')) do
  context "default fcgid config" do
    it 'succeeds in puppeting fcgid' do
      pp = <<-EOS
        class { 'epel': } # mod_fcgid lives in epel
        class { 'apache': }
        class { 'apache2::mod::php': } # For /usr/bin/php-cgi
        class { 'apache2::mod::fcgid':
          options => {
            'FcgidIPCDir'  => '/var/run/fcgidsock',
          },
        }
        apache2::vhost { 'fcgid.example.com':
          port        => '80',
