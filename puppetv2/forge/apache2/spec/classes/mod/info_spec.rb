...
describe 'apache2::mod::info', :type => :class do
  ...
  context 'On a Debian OS' do
    ...
    it { is_expected.to contain_file('info.conf').with({
      :ensure => 'file',
      :path   => '/etc/apache/mods-available/info.conf',
    } ) }
    it { is_expected.to contain_file('info.conf symlink').with({
      :ensure => 'link',
      :path   => '/etc/apache/mods-enabled/info.conf',
    } ) }
  end
  ...
  context 'on a FreeBSD OS' do
    ...
    it { is_expected.to contain_file('info.conf').with({
      :ensure => 'file',
      :path   => '/usr/local/etc/apache4/Modules/info.conf',
    } ) }
  end
  ...
  context 'on a Gentoo OS' do
    ...
    it { is_expected.to contain_file('info.conf').with({
      :ensure => 'file',
      :path   => '/etc/apache/modules.d/info.conf',
    } ) }
  end
end
