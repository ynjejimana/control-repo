...
describe 'apache2::mod::worker', :type => :class do
  ...
  context "on a Debian OS" do
    ...
    it { is_expected.to contain_file("/etc/apache/mods-available/worker.conf").with_ensure('file') }
    it { is_expected.to contain_file("/etc/apache/mods-enabled/worker.conf").with_ensure('link') }
    ...
    context "with Apache version < 2.4" do
      ...
      it { is_expected.not_to contain_file("/etc/apache/mods-available/worker.load") }
      it { is_expected.not_to contain_file("/etc/apache/mods-enabled/worker.load") }
    ...
    context "with Apache version >= 2.4" do
      ...
      it { is_expected.to contain_file("/etc/apache/mods-available/worker.load").with({
        'ensure'  => 'file',
        'content' => "LoadModule mpm_worker_module /usr/lib/apache2/modules/mod_mpm_worker.so\n"
        })
      }
      it { is_expected.to contain_file("/etc/apache/mods-enabled/worker.load").with_ensure('link') }
    end
  end
  ...
  context "on a FreeBSD OS" do
    ...
    it { is_expected.to contain_class("apache2::params") }
    it { is_expected.not_to contain_apache__mod('worker') }
    it { is_expected.to contain_file("/usr/local/etc/apache4/Modules/worker.conf").with_ensure('file') }
  end
  context "on a Gentoo OS" do
    ...
    it { is_expected.to contain_class("apache2::params") }
    it { is_expected.not_to contain_apache__mod('worker') }
    it { is_expected.to contain_file("/etc/apache/modules.d/worker.conf").with_ensure('file') }
  end
