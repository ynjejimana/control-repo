...
describe 'apache2::mod::deflate', :type => :class do
  ...
  context "default configuration with parameters" do
    ...
    context "On a Debian OS with default params" do
      ...
      it { is_expected.to contain_file("deflate.conf").with({
        :ensure => 'file',
        :path   => '/etc/apache/mods-available/deflate.conf',
      } ) }
      it { is_expected.to contain_file("deflate.conf symlink").with({
        :ensure => 'link',
        :path   => '/etc/apache/mods-enabled/deflate.conf',
      } ) }
    end
    ...
    context "On a FreeBSD OS with default params" do
      ...
      it { is_expected.to contain_file("deflate.conf").with({
        :ensure => 'file',
        :path   => '/usr/local/etc/apache4/Modules/deflate.conf',
      } ) }
    end
    ...
    context "On a Gentoo OS with default params" do
      ...
      it { is_expected.to contain_file("deflate.conf").with({
        :ensure => 'file',
        :path   => '/etc/apache/modules.d/deflate.conf',
      } ) }
    end
  end
end
