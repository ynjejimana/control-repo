...
describe 'apache2::mod::rpaf', :type => :class do
  ...
  context "on a Debian OS" do
    ...
    it { is_expected.to contain_file('rpaf.conf').with({
      'path' => '/etc/apache/mods-available/rpaf.conf',
    }) }
  ...
  context "on a FreeBSD OS" do
    ...
    it { is_expected.to contain_file('rpaf.conf').with({
      'path' => '/usr/local/etc/apache4/Modules/rpaf.conf',
    }) }
    it { is_expected.to contain_file('rpaf.conf').with_content(/^RPAFenable On$/) }
  ...
  context "on a Gentoo OS" do
    ...
    it { is_expected.to contain_file('rpaf.conf').with({
      'path' => '/etc/apache/modules.d/rpaf.conf',
    }) }
    it { is_expected.to contain_file('rpaf.conf').with_content(/^RPAFenable On$/) }
