...
define apache2::custom_config (
  $ensure         = 'present',
  $confdir        = $::apache2::confd_dir,
  $content        = undef,
  $priority       = '25',
  $source         = undef,
  $verify_command = $::apache2::params::verify_command,
  $verify_config  = true,
  $filename       = undef,
) {
  ...
  if ! $verify_config or $ensure == 'absent' {
    $notifies = Class['Apache2::Service']
  } else {
    $notifies = undef
  }
  ...
  if $ensure == 'present' and $verify_config {
    exec { "syntax verification for ${name}":
      ...
      require     => Anchor['::apache2::modules_set_up']
    }
