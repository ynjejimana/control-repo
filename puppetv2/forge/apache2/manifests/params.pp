...
# Sample Usage:
#
class apache2::params inherits ::apache2::version {
  if($::fqdn) {
    $servername = $::fqdn
  } else {
    $servername = $::hostname
  ...
  if $::osfamily == 'RedHat' or $::operatingsystem =~ /^[Aa]mazon$/ {
    ...
    $mod_dir              = $::apache2::version::distrelease ? {
    ...
    $mod_packages         = {
      ...
      'authnz_ldap' => $::apache2::version::distrelease ? {
        '7'     => 'mod_ldap',
      ...
      'fcgid'       => 'mod_fcgid',
      'geoip'       => 'mod_geoip',
      'ldap'        => $::apache2::version::distrelease ? {
        '7'     => 'mod_ldap',
        default => undef,
      },
      ...
      # repositories. See
      # https://www.phusionpassenger.com/library/install/apache2/install/oss/el7/
      'passenger'   => 'mod_passenger',
      'perl'        => 'mod_perl',
      'php5'        => $::apache2::version::distrelease ? {
        '5'     => 'php53',
        default => 'php',
      },
      'phpXXX'      => 'php',
    ...
    $conf_template        = 'apache2/httpd.conf.erb'
    $keepalive            = 'Off'
    ...
    $alias_icons_path     = $::apache2::version::distrelease ? {
      '7'     => '/usr/share/httpd/icons',
      default => '/var/www/icons',
    }
    $error_documents_path = $::apache2::version::distrelease ? {
    ...
    $httpd_dir        = '/usr/local/etc/apache4'
    ...
    $mpm_module       = 'prefork'
    $dev_packages     = undef
    $default_ssl_cert = '/usr/local/etc/apache4/server.crt'
    $default_ssl_key  = '/usr/local/etc/apache4/server.key'
    $ssl_certs_dir    = '/usr/local/etc/apache4'
    $passenger_conf_file = 'passenger.conf'
    $passenger_conf_package_file = undef
    ...
    fail("Class['apache2::params']: Unsupported osfamily: ${::osfamily}")
  }
}
