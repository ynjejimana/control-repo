define apache2::mpm (
  ...
  case $::osfamily {
    'debian': {
      file { "${::apache2::mod_enable_dir}/${mpm}.conf":
        ensure  => link,
        target  => "${::apache2::mod_dir}/${mpm}.conf",
        require => Exec["mkdir ${::apache2::mod_enable_dir}"],
        before  => File[$::apache2::mod_enable_dir],
        notify  => Class['apache2::service'],
      }

      if versioncmp($apache_version, '2.4') >= 0 {
        file { "${::apache2::mod_enable_dir}/${mpm}.load":
          ensure  => link,
          target  => "${::apache2::mod_dir}/${mpm}.load",
          require => Exec["mkdir ${::apache2::mod_enable_dir}"],
          before  => File[$::apache2::mod_enable_dir],
          notify  => Class['apache2::service'],
        }
      ...
      if $mpm == 'itk' and $::operatingsystem == 'Ubuntu' and $::operatingsystemrelease == '14.04' {
        ...
        exec {
          '/usr/sbin/a2dismod mpm_event':
            onlyif  => '/usr/bin/test -e /etc/apache/mods-enabled/mpm_event.load',
            require => Package['httpd'],
