...
define apache2::vhost(
  ...
  $add_listen                  = true,
  $docroot_owner               = 'root',
  $docroot_group               = $::apache2::params::root_group,
  $docroot_mode                = undef,
  $serveradmin                 = undef,
  $ssl                         = false,
  $ssl_cert                    = $::apache2::default_ssl_cert,
  $ssl_key                     = $::apache2::default_ssl_key,
  $ssl_chain                   = $::apache2::default_ssl_chain,
  $ssl_ca                      = $::apache2::default_ssl_ca,
  $ssl_crl_path                = $::apache2::default_ssl_crl_path,
  $ssl_crl                     = $::apache2::default_ssl_crl,
  $ssl_crl_check               = $::apache2::default_ssl_crl_check,
  $ssl_certs_dir               = $::apache2::params::ssl_certs_dir,
  $ssl_protocol                = undef,
  $ssl_cipher                  = undef,
  ...
  $suphp_addhandler            = $::apache2::params::suphp_addhandler,
  $suphp_engine                = $::apache2::params::suphp_engine,
  $suphp_configpath            = $::apache2::params::suphp_configpath,
  $php_flags                   = {},
  $php_values                  = {},
  $php_admin_flags             = {},
  $php_admin_values            = {},
  ...
  $additional_includes         = [],
  $use_optional_includes       = $::apache2::use_optional_includes,
  ...
  $apache_name = $::apache2::apache_name
  ...
  if $rewrite_base {
    warning('Apache2::Vhost: parameter rewrite_base is deprecated in favor of rewrites')
  }
  if $rewrite_rule {
    warning('Apache2::Vhost: parameter rewrite_rule is deprecated in favor of rewrites')
  }
  if $rewrite_cond {
    warning('Apache2::Vhost parameter rewrite_cond is deprecated in favor of rewrites')
  }
  ...
  if $access_log_file and $access_log_pipe {
    fail("Apache2::Vhost[${name}]: 'access_log_file' and 'access_log_pipe' cannot be defined at the same time")
  }

  if $error_log_file and $error_log_pipe {
    fail("Apache2::Vhost[${name}]: 'error_log_file' and 'error_log_pipe' cannot be defined at the same time")
  }
  ...
  # Is apache2::mod::passenger enabled (or apache2::mod['passenger'])
  $passenger_enabled = defined(Apache2::Mod['passenger'])

  # Is apache2::mod::shib enabled (or apache2::mod['shib2'])
  $shibboleth_enabled = defined(Apache2::Mod['shib2'])
  ...
  if $access_log and !$access_logs {
    ...
    if !is_array($access_logs) {
      fail("Apache2::Vhost[${name}]: access_logs must be an array of hashes")
    }
  ...
  if $ip {
    ...
    if $port {
      ...
      if ! $servername and ! $ip_based {
        fail("Apache2::Vhost[${name}]: must pass 'ip' and/or 'port' parameters for name-based vhosts")
      }
    }
  } else {
    if $port {
      ...
      $nvh_addr_port = $name
      if ! $servername {
        fail("Apache2::Vhost[${name}]: must pass 'ip' and/or 'port' parameters, and/or 'servername' parameter")
      }
    }
  }
  if $add_listen {
    if $ip and defined(Apache2::Listen["${port}"]) {
      fail("Apache2::Vhost[${name}]: Mixing IP and non-IP Listen directives is not possible; check the add_listen parameter of the apache2::vhost define to disable this")
    }
    if $listen_addr_port and $ensure == 'present' {
      ensure_resource('apache2::listen', $listen_addr_port)
    }
  }
  if ! $ip_based {
    if $ensure == 'present' and (versioncmp($apache_version, '2.4') < 0) {
      ensure_resource('apache2::namevirtualhost', $nvh_addr_port)
    }
  }

  # Load mod_rewrite if needed and not yet loaded
  if $rewrites or $rewrite_cond {
    if ! defined(Class['apache2::mod::rewrite']) {
  ...
  if ($scriptalias or $scriptaliases != []) or ($aliases and $aliases != []) or ($redirect_source and $redirect_dest) {
    if ! defined(Class['apache2::mod::alias'])  and ($ensure == 'present') {
  ...
  if ($proxy_dest or $proxy_pass or $proxy_pass_match or $proxy_dest_match) {
    if ! defined(Class['apache2::mod::proxy']) {
      include ::apache2::mod::proxy
    }
    if ! defined(Class['apache2::mod::proxy_http']) {
  ...
  if $rack_base_uris {
    if ! defined(Class['apache2::mod::passenger']) {
      include ::apache2::mod::passenger
    }
  }
  ...
  if $fastcgi_server and $fastcgi_socket {
    if ! defined(Class['apache2::mod::fastcgi']) {
  ...
  if $headers or $request_headers {
    if ! defined(Class['apache2::mod::headers']) {
  ...
  if $filters {
    if ! defined(Class['apache2::mod::filter']) {
  ...
  if ($use_setenv_mod) {
    if ! defined(Class['apache2::mod::setenvif']) {
  ...
  if $directories {
    if !is_hash($directories) and !(is_array($directories) and is_hash($directories[0])) {
      fail("Apache2::Vhost[${name}]: 'directories' must be either a Hash or an Array of Hashes")
    }
  ...
  if $modsec_disable_ids {
    if is_hash($modsec_disable_ids) {
      ...
      fail("Apache2::Vhost[${name}]: 'modsec_disable_ids' must be either a Hash of location/IDs or an Array of IDs")
    }
  }
  ...
  if $modsec_disable_msgs {
    if is_hash($modsec_disable_msgs) {
      ...
      fail("Apache2::Vhost[${name}]: 'modsec_disable_msgs' must be either a Hash of location/Msgs or an Array of Msgs")
    }
  }
  ...
  if $modsec_disable_tags {
    if is_hash($modsec_disable_tags) {
      ...
      fail("Apache2::Vhost[${name}]: 'modsec_disable_tags' must be either a Hash of location/Tags or an Array of Tags")
    }
  }

  concat { "${priority_real}${filename}.conf":
    #ensure  => $ensure,
    path    => "${::apache2::vhost_dir}/${priority_real}${filename}.conf",
    owner   => 'root',
  ...
  # NOTE(pabelanger): This code is duplicated in ::apache2::vhost::custom and
  # needs to be converted into something generic.
  if $::apache2::vhost_enable_dir {
    $vhost_enable_dir = $::apache2::vhost_enable_dir
    ...
    file{ "${priority_real}${filename}.conf symlink":
      ...
      target  => "${::apache2::vhost_dir}/${priority_real}${filename}.conf",
      owner   => 'root',
      group   => $::apache2::params::root_group,
      mode    => $::apache2::file_mode,
  ...
  concat::fragment { "${name}-apache-header":
    ...
    order   => 0,
    content => template('apache2/vhost/_file_header.erb'),
  }
  ...
  if $docroot {
    concat::fragment { "${name}-docroot":
      target  => "${priority_real}${filename}.conf",
      order   => 10,
      content => template('apache2/vhost/_docroot.erb'),
    }
  }
  ...
  if $aliases and ! empty($aliases) {
    concat::fragment { "${name}-aliases":
      target  => "${priority_real}${filename}.conf",
      order   => 20,
      content => template('apache2/vhost/_aliases.erb'),
    }
  }
  ...
  if $itk and ! empty($itk) {
    concat::fragment { "${name}-itk":
      target  => "${priority_real}${filename}.conf",
      order   => 30,
      content => template('apache2/vhost/_itk.erb'),
    }
  }
  ...
  if $fallbackresource {
    concat::fragment { "${name}-fallbackresource":
      target  => "${priority_real}${filename}.conf",
      order   => 40,
      content => template('apache2/vhost/_fallbackresource.erb'),
    }
  }
  ...
  if $allow_encoded_slashes {
    concat::fragment { "${name}-allow_encoded_slashes":
      target  => "${priority_real}${filename}.conf",
      order   => 50,
      content => template('apache2/vhost/_allow_encoded_slashes.erb'),
    }
  }
  ...
  if $_directories and ! empty($_directories) {
    concat::fragment { "${name}-directories":
      target  => "${priority_real}${filename}.conf",
      order   => 60,
      content => template('apache2/vhost/_directories.erb'),
    }
  }
  ...
  if $additional_includes and ! empty($additional_includes) {
    concat::fragment { "${name}-additional_includes":
      target  => "${priority_real}${filename}.conf",
      order   => 70,
      content => template('apache2/vhost/_additional_includes.erb'),
    }
  }
  ...
  if $error_log or $log_level {
    concat::fragment { "${name}-logging":
      target  => "${priority_real}${filename}.conf",
      order   => 80,
      content => template('apache2/vhost/_logging.erb'),
    }
  }
  ...
  concat::fragment { "${name}-serversignature":
    target  => "${priority_real}${filename}.conf",
    order   => 90,
    content => template('apache2/vhost/_serversignature.erb'),
  }
  ...
  if $access_log or $access_logs {
    concat::fragment { "${name}-access_log":
      target  => "${priority_real}${filename}.conf",
      order   => 100,
      content => template('apache2/vhost/_access_log.erb'),
    }
  }
  ...
  if $action {
    concat::fragment { "${name}-action":
      target  => "${priority_real}${filename}.conf",
      order   => 110,
      content => template('apache2/vhost/_action.erb'),
    }
  }
  ...
  if $block and ! empty($block) {
    concat::fragment { "${name}-block":
      target  => "${priority_real}${filename}.conf",
      order   => 120,
      content => template('apache2/vhost/_block.erb'),
    }
  }
  ...
  if $error_documents and ! empty($error_documents) {
    concat::fragment { "${name}-error_document":
      target  => "${priority_real}${filename}.conf",
      order   => 130,
      content => template('apache2/vhost/_error_document.erb'),
    }
  }
  ...
  if $headers and ! empty($headers) {
    concat::fragment { "${name}-header":
      target  => "${priority_real}${filename}.conf",
      order   => 140,
      content => template('apache2/vhost/_header.erb'),
    }
  }
  ...
  if $request_headers and ! empty($request_headers) {
    concat::fragment { "${name}-requestheader":
      target  => "${priority_real}${filename}.conf",
      order   => 150,
      content => template('apache2/vhost/_requestheader.erb'),
    }
  }
  ...
  if $proxy_dest or $proxy_pass or $proxy_pass_match or $proxy_dest_match {
    concat::fragment { "${name}-proxy":
      target  => "${priority_real}${filename}.conf",
      order   => 160,
      content => template('apache2/vhost/_proxy.erb'),
    }
  }
  ...
  if $rack_base_uris {
    concat::fragment { "${name}-rack":
      target  => "${priority_real}${filename}.conf",
      order   => 170,
      content => template('apache2/vhost/_rack.erb'),
    }
  }
  ...
  if $passenger_base_uris {
    concat::fragment { "${name}-passenger_uris":
      target  => "${priority_real}${filename}.conf",
      order   => 175,
      content => template('apache2/vhost/_passenger_base_uris.erb'),
    }
  }
  ...
  if ($redirect_source and $redirect_dest) or ($redirectmatch_regexp and $redirectmatch_dest) {
    concat::fragment { "${name}-redirect":
      target  => "${priority_real}${filename}.conf",
      order   => 180,
      content => template('apache2/vhost/_redirect.erb'),
    }
  }
  ...
  if $rewrites or $rewrite_rule {
    concat::fragment { "${name}-rewrite":
      target  => "${priority_real}${filename}.conf",
      order   => 190,
      content => template('apache2/vhost/_rewrite.erb'),
    }
  }
  ...
  if ( $scriptalias or $scriptaliases != [] ) {
    concat::fragment { "${name}-scriptalias":
      target  => "${priority_real}${filename}.conf",
      order   => 200,
      content => template('apache2/vhost/_scriptalias.erb'),
    }
  }
  ...
  if $serveraliases and ! empty($serveraliases) {
    concat::fragment { "${name}-serveralias":
      target  => "${priority_real}${filename}.conf",
      order   => 210,
      content => template('apache2/vhost/_serveralias.erb'),
    }
  }
  ...
  if ($use_setenv_mod) {
    concat::fragment { "${name}-setenv":
      target  => "${priority_real}${filename}.conf",
      order   => 220,
      content => template('apache2/vhost/_setenv.erb'),
    }
  }
  ...
  if $ssl {
    concat::fragment { "${name}-ssl":
      target  => "${priority_real}${filename}.conf",
      order   => 230,
      content => template('apache2/vhost/_ssl.erb'),
    }
  }
  ...
  if $ssl_proxyengine {
    concat::fragment { "${name}-sslproxy":
      target  => "${priority_real}${filename}.conf",
      order   => 230,
      content => template('apache2/vhost/_sslproxy.erb'),
    }
  }
  ...
  if $auth_kerb {
    concat::fragment { "${name}-auth_kerb":
      target  => "${priority_real}${filename}.conf",
      order   => 230,
      content => template('apache2/vhost/_auth_kerb.erb'),
    }
  }
  ...
  if $suphp_engine == 'on' {
    concat::fragment { "${name}-suphp":
      target  => "${priority_real}${filename}.conf",
      order   => 240,
      content => template('apache2/vhost/_suphp.erb'),
    }
  }
  ...
  if ($php_values and ! empty($php_values)) or ($php_flags and ! empty($php_flags)) {
    concat::fragment { "${name}-php":
      target  => "${priority_real}${filename}.conf",
      order   => 240,
      content => template('apache2/vhost/_php.erb'),
    }
  }
  ...
  if ($php_admin_values and ! empty($php_admin_values)) or ($php_admin_flags and ! empty($php_admin_flags)) {
    concat::fragment { "${name}-php_admin":
      target  => "${priority_real}${filename}.conf",
      order   => 250,
      content => template('apache2/vhost/_php_admin.erb'),
    }
  }
  ...
  if $wsgi_application_group or $wsgi_daemon_process or ($wsgi_import_script and $wsgi_import_script_options) or $wsgi_process_group or ($wsgi_script_aliases and ! empty($wsgi_script_aliases)) or $wsgi_pass_authorization {
    concat::fragment { "${name}-wsgi":
      target  => "${priority_real}${filename}.conf",
      order   => 260,
      content => template('apache2/vhost/_wsgi.erb'),
    }
  }
  ...
  if $custom_fragment {
    concat::fragment { "${name}-custom_fragment":
      target  => "${priority_real}${filename}.conf",
      order   => 270,
      content => template('apache2/vhost/_custom_fragment.erb'),
    }
  }
  ...
  if $fastcgi_server or $fastcgi_dir {
    concat::fragment { "${name}-fastcgi":
      target  => "${priority_real}${filename}.conf",
      order   => 280,
      content => template('apache2/vhost/_fastcgi.erb'),
    }
  }
  ...
  if $suexec_user_group {
    concat::fragment { "${name}-suexec":
      target  => "${priority_real}${filename}.conf",
      order   => 290,
      content => template('apache2/vhost/_suexec.erb'),
    }
  }
  ...
  if $passenger_app_root or $passenger_app_env or $passenger_ruby or $passenger_min_instances or $passenger_start_timeout or $passenger_pre_start or $passenger_user {
    concat::fragment { "${name}-passenger":
      target  => "${priority_real}${filename}.conf",
      order   => 300,
      content => template('apache2/vhost/_passenger.erb'),
    }
  }
  ...
  if $add_default_charset {
    concat::fragment { "${name}-charsets":
      target  => "${priority_real}${filename}.conf",
      order   => 310,
      content => template('apache2/vhost/_charsets.erb'),
    }
  }
  ...
  if $modsec_disable_vhost or $modsec_disable_ids or $modsec_disable_ips or $modsec_disable_msgs or $modsec_disable_tags {
    concat::fragment { "${name}-security":
      target  => "${priority_real}${filename}.conf",
      order   => 320,
      content => template('apache2/vhost/_security.erb')
    }
  }
  ...
  if $filters and ! empty($filters) {
    concat::fragment { "${name}-filters":
      target  => "${priority_real}${filename}.conf",
      order   => 330,
      content => template('apache2/vhost/_filters.erb'),
    }
  }
  ...
  if $jk_mounts and ! empty($jk_mounts) {
    concat::fragment { "${name}-jk_mounts":
      target  => "${priority_real}${filename}.conf",
      order   => 340,
      content => template('apache2/vhost/_jk_mounts.erb'),
    }
  }
  ...
  if $keepalive or $keepalive_timeout or $max_keepalive_requests {
    concat::fragment { "${name}-keepalive_options":
      target  => "${priority_real}${filename}.conf",
      order   => 350,
      content => template('apache2/vhost/_keepalive_options.erb'),
    }
  }
  ...
  concat::fragment { "${name}-file_footer":
    target  => "${priority_real}${filename}.conf",
    order   => 999,
    content => template('apache2/vhost/_file_footer.erb'),
  }
}
