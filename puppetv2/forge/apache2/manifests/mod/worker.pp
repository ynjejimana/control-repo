...
#  [*apache_version*]
#   (optional)
#   Default is $::apache2::apache_version
#
class apache2::mod::worker (
  $startservers        = '2',
  $maxclients          = '150',
  ...
  if defined(Class['apache2::mod::event']) {
    fail('May not include both apache2::mod::worker and apache2::mod::event on the same node')
  }
  if defined(Class['apache2::mod::itk']) {
    fail('May not include both apache2::mod::worker and apache2::mod::itk on the same node')
  }
  if defined(Class['apache2::mod::peruser']) {
    fail('May not include both apache2::mod::worker and apache2::mod::peruser on the same node')
  }
  if defined(Class['apache2::mod::prefork']) {
    fail('May not include both apache2::mod::worker and apache2::mod::prefork on the same node')
  }
  File {
    owner => 'root',
  ...
  # - $threadLimit
  # - $listenbacklog
  file { "${::apache2::mod_dir}/worker.conf":
    ensure  => file,
    content => template('apache2/mod/worker.conf.erb'),
