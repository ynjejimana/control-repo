class apache2::mod::disk_cache (
  ...
  elsif versioncmp($::apache2::apache_version, '2.4') >= 0 {
  ...
  if versioncmp($::apache2::apache_version, '2.4') >= 0 {
    apache2::mod { 'cache_disk': }
  }
  else {
    apache2::mod { 'disk_cache': }
  }

  Class['::apache2::mod::cache'] -> Class['::apache2::mod::disk_cache']

  # Template uses $_cache_root
  file { 'disk_cache.conf':
    ensure  => file,
    path    => "${::apache2::mod_dir}/disk_cache.conf",
    mode    => $::apache2::file_mode,
    content => template('apache2/mod/disk_cache.conf.erb'),
