...
class os_hardening(
  ...
  $root_ttys                =
    ['console','tty1','tty2','tty3','tty4','tty5','tty6','serial_console'],
...
#  $disable_services = ['xinetd','telnet-server', 'rsh-server', 'telnet', 'rsh',
#		    'ypbind', 'ypserv', 'tftp-server', 'cronie-anacron', 'bind',
#		    'vsftpd', 'httpd', 'dovecot', 'squid', 'net-snmpd'],
  $disable_services = ['xinetd','telnet-server', 'rsh-server', 'telnet', 'rsh',
		    'ypbind', 'ypserv', 'tftp-server',
		    'vsftpd', 'dovecot', 'squid', 'net-snmpd'],
  ...
  class {'os_hardening::selinux':
  	mode => $selinux_mode,
	type   => $selinux_type,
  }

  $services = split(inline_template('<%= disable_services - protected_packages =>'),',')
