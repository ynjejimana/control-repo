#!/usr/bin/perl
#### Semi-automated Zabbix Proxy core functionality installer (for Zabbix Proxy API)
#### Version 1.5
...
our $coreScriptsDestinationDir = "/srv/zabbix/scripts/zabbixproxyapi";
...
my $installerLockFile =
  $installerLocksDir . "/2448ba77-7888-4e8d-b7e7-e7cb523ff10f";
...
## install monitoring script
	...
	$tmpFileName = 'zabbix-proxy-api-server.pl';
...
## install monitoring script
	...
	$tmpFileName = 'zabbix-proxy-api-server.pl-forZabbixProxy1.8';
...
## - set all permissions to files in the script directory
	...
	my ( $execSubResult, $execResultCode, $execStdErr, @execOutput ) =
	  installerExecuteShellCommand(
		'chmod 775 ' . $coreScriptsDestinationDir . "/*.pl" );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}
