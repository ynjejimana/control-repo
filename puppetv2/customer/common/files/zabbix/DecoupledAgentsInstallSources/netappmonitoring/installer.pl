#!/usr/bin/perl
#### Semi-automated Zabbix Proxy core functionality installer (for Netapp Monitoring V1 Proxy)
#### Version 1.11
...
our $coreScriptsDestinationDir = "/srv/zabbix/scripts/netappmonitoring";
...
my $installerLockFile =
  $installerLocksDir . "/77c5d48e-4f77-4ff5-9654-c55038588ee2";
...
## - create core scripts directory
	installerMakePath( 1, $coreScriptsDestinationDir );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}
...
## install monitoring scripts
	...
	my ( $execSubResult, $execResultCode, $execStdErr, @execOutput ) =
	  installerExecuteShellCommand( 'cp -r '
		  . $installerTempDir
		  . '/corescripts/* '
		  . $coreScriptsDestinationDir );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}
...
## - set all permissions to files in the script directory
	...
	# - permissions to executable files 1
	...
	# - permissions to executable files 2

	my ( $execSubResult, $execResultCode, $execStdErr, @execOutput ) =
	  installerExecuteShellCommand(
		'chmod 774 ' . $coreScriptsDestinationDir . "/*.sh" );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}
