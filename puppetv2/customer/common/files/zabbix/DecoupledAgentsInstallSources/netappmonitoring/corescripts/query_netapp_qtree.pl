...
sub main {
	...
	if ( scalar( keys %$first ) == 0 ) {
		print "Qtree " . @{ $opts->{'qtree'} }[0] . " not found!\n";
		exit(1);
	}
	...
	foreach (@{$opts->{'qtree'}})
		...
		if ($second->{'ops-'.$qtree} > $first->{'ops-'.$qtree}) {
			$total_ops = $second->{'ops-'.$qtree} - $first->{'ops-'.$qtree};
		}
 
		push(@values, $total_ops);
...
sub parse_options {
	...
	# into an array here...
	@{$options{ 'qtree' }} = split( /,/, join( ',', @{$options{ 'qtree' }} ) );
	...
	if ( ! $options{ 'hostname' }              || ! $options{ 'username' } || 
	     scalar( @{$options{ 'qtree' }} ) < 1 || $options{ 'help' }       ||
...
#REMOVE ME
	...
	if ( scalar( $result->child_get('instances')->children_get() ) > 0 ) 
	{
		...
		{	
			...
			$qtree = $_->child_get_string('name');
			$return->{ 'qtree-'.$qtree } = $qtree;
			...
			foreach ($instance_data->child_get('counters')->children_get()) 
			{
				...
				$return->{ $name."-".$qtree } = $_->child_get_string('value');
			}
		}
	}
...
#################### qtree
        	$request->child_add_string('objectname', 'qtree');
...
#		$instances->child_add_string('instance', 'nrlab_GOE_DB_u01/nrlab_goepsdb02_GOE_DB_u01');
