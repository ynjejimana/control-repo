...
sub main {
	...
	while (1) {
		...
		if ($opts->{'average'} && $second->{'total_ops'} > $first->{'total_ops'}) {
			$total_ops = $second->{'total_ops'} - $first->{'total_ops'};
			$avg_latency = ($second->{'avg_latency'} - $first->{'avg_latency'}) / $total_ops;
		}
		...
		if ($opts->{'read'} && $second->{'read_ops'} > $first->{'read_ops'}) {
			$read_ops = $second->{'read_ops'} - $first->{'read_ops'};
			$read_latency = ($second->{'read_latency'} - $first->{'read_latency'}) / $read_ops;
		}
 
		push(@values, $read_latency) if $opts->{'read'};
		push(@values, $read_ops) if $opts->{'ops'} && $opts->{'read'};
 
		if ($opts->{'write'} && $second->{'write_ops'} > $first->{'write_ops'}) {
			$write_ops = $second->{'write_ops'} - $first->{'write_ops'};
			$write_latency = ($second->{'write_latency'} - $first->{'write_latency'}) / $write_ops;
		}
		...
		if ($opts->{'other'} && $second->{'other_ops'} > $first->{'other_ops'}) {
			$other_ops = $second->{'other_ops'} - $first->{'other_ops'};
			$other_latency = ($second->{'other_latency'} - $first->{'other_latency'}) / $other_ops;
		}
...
sub get_filer {
	...
	$s->set_transport_type(NA_SERVER_TRANSPORT_HTTPS);
...
sub build_request {
	...
	$instances->child_add_string('instance', @{$opts->{'volume'}}[0]);
