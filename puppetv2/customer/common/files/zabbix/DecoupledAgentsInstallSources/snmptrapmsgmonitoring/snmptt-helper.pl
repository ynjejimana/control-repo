...
my $thisProxyZabbixVersionMajor;    # 1 or 3 - supported Zabbix versions
...
$zabbix_server = readSettingFromDatabase1("DESTINATION_ZABBIX_PROXY");

if ( !defined($zabbix_server) ) {
	logEvent( 2, "DESTINATION_ZABBIX_PROXY does not exist in TA_SETTINGS. Terminating." );
	exit 1;
}
...
eval {
	...
	$trapProxyMessageId = writeSNMPTrapMsgToDatabase2(
		...
		$lastSendTrapToZabbixOperationErrorMessage,$zabbix_server.":".$zabbix_port
	) || exit 1;
...
sub SendTrapToZabbix {
	...
	my $returnBuff;
	my $errorMessage = 'ok';
	my $zabbixTrapperReturnStringOK;
	
	
	if ($thisProxyZabbixVersionMajor==1){
	    $zabbixTrapperReturnStringOK='Processed 1 Failed 0 Total 1 Seconds spent';
	} else {
	    $zabbixTrapperReturnStringOK='processed: 1; failed: 0; total: 1; seconds spent';
	}

	
	$lastSendTrapToZabbixOperationResultCode = 0;
	...
	if (   ( $tmpExecResult == 0 )
		&& ( $returnBuff =~ /$zabbixTrapperReturnStringOK/ ) )
	{

		logEvent( 1, "Sending was successful." );
		...
		$errorMessage =
		    "ERROR: Failed to send trap item. EXIT CODE: "
		  . $tmpExecResult
		  . ", CMD: $cmd , RETURN BUFFER: $returnBuff";

		$lastSendTrapToZabbixOperationErrorMessage=$errorMessage;
		logEvent( 2, $errorMessage );

	}

		
	return ( $lastSendTrapToZabbixOperationResultCode);
	
}



sub SendTrapToZabbixOLD {
	my ( $zabbixHost, $key, $msg ) = @_;
	my $result = 0;
	my $returnBuff;
...
{    # Saving step 2: returns 0, or 1 if operation was successful
	...
	my ( $recordID, $zabbixHost, $zabbixMessageQueue, $zabbixSendingResult,
		$zabbixSendingErrorMessage,$zabbixProxy )
	  = @_;
	my $subresult = 0;
	my $sql1;
	my $sth1;
...
UPDATE TA_RECEIVED_TRAP_MSGS SET DS_ZABBIX_SENDER_FORWARDING_HOST=?,DS_ZABBIX_SENDER_FORWARDING_QUEUE=?,
DL_ZABBIX_SENDER_FORWARDING_RESULT_CODE=?, DS_ZABBIX_SENDER_FORWARDING_ERROR_DESCRIPTION=?, DS_ZABBIX_SENDER_FORWARDING_PROXY=?
WHERE DL_ID=?
...
$sth1 = $localDatabaseDBH->prepare($sql1);
$sth1->execute($zabbixHost,$zabbixMessageQueue,$zabbixSendingResult,$zabbixSendingErrorMessage,$zabbixProxy,$recordID);
...
#$localDatabaseDBH->commit();
	...
	if ($@) {
		...
		logEvent( 2, "Error 1 occured in writeSNMPTrapMsgToDatabase2(): $@ . DBI::errstr: ".$DBI::errstr );
		eval { $localDatabaseDBH->rollback(); };
