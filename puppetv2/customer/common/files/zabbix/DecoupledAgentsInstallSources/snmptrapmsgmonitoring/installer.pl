#!/usr/bin/perl
#### Semi-automated Zabbix Proxy core functionality installer (for SNMP Trapper V2 Proxy)
#### Version 1.32
...
our $coreScriptsDestinationDir = "/srv/zabbix/scripts/snmptrapmsgmonitoring";
our $zabbixWebsiteScriptsDestinationDir = "/var/www/html/snmptrapmsgmonitoring";
our $snmpConfigDir                      = "/etc/snmp";
...
my $installerLockFile =
  $installerLocksDir . "/288d3c9c-4ddd-4cea-888d-06656294177c";
...
#	my $cpanExtraModules1 = ' Net::SNMP Data::GUID';
...
## install monitoring script
	...
	$tmpFileName = 'find-mib-per-specs.pl';
...
## install snmptt config file

	$tmpFileName = 'snmptt.ini';

	installerCopyFile(
		1,
		$installerTempDir . '/' . $tmpFileName,
		$snmpConfigDir . '/' . $tmpFileName
	);
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}
...
## - install website scripts
	...
	$tmpFileName = 'snmptrappergui.php';
...
## - install website templates (TrapMsgForwardRule-HostTemplates)

	# create trapper forwarding rule template directory

	my $templatesDirName = 'TrapMsgForwardRule-HostTemplates';

	installerMakePath( 1,
		$zabbixWebsiteScriptsDestinationDir . '/' . $templatesDirName,0 );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}

	# - deploy trapper forwarding rule templates

	my ( $execSubResult, $execResultCode, $execStdErr, @execOutput ) =
	  installerExecuteShellCommand( 'cp '
		  . $installerTempDir . '/'
		  . $templatesDirName . '/* '
		  . $zabbixWebsiteScriptsDestinationDir . '/'
		  . $templatesDirName );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}
...
## - create core scripts database set
	...
	my ( $execSubResult, $execResultCode, $execStdErr, @execOutput ) =
	  installerExecuteShellCommand( $coreScriptsDestinationDir
		  . '/diag-n-maintain.pl SETTING SET ZABBIX_VERSION_MAJOR 1' )
	  ;    # this setting must be set to correct major version of Zabbix used
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}
...
## - set permissions to the script directory
	...
	my ( $execSubResult, $execResultCode, $execStdErr, @execOutput ) =
	  installerExecuteShellCommand( 'chmod 775 ' . $coreScriptsDestinationDir );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}
...
## - set all permissions to files in the script directory
	...
	my ( $execSubResult, $execResultCode, $execStdErr, @execOutput ) =
	  installerExecuteShellCommand(
		'chmod 664 ' . $coreScriptsDestinationDir . "/*" );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}
