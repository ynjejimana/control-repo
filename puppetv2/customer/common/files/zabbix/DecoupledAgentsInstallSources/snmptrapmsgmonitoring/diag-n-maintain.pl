...
if ( scalar(@ARGV) < 1 ) {
	...
	if ( uc( $ARGV[0] ) eq "DATA" ) {

		if ( uc( $ARGV[1] ) eq "FETCHPROXYHOSTIPMAPPINGS" ) {
...
## open db
		...
		if ( !-f $databaseFileName ) {
			...
			logEvent( 2,
"Error 4 occurred in fnFetchProxyConfigData1(). Database file $databaseFileName does not exist"
			);
		...
		$dbh = DBI->connect(
		  ...
		  (
			...
			logEvent( 2,
"Error 5 occured in fnFetchProxyConfigData1(): Can't open database: $DBI::errstr"
			)
			&& return $subResult );

## read settings

		readSettingFromDatabase1( "DESTINATION_ZABBIX_PROXY",
			\$sockedRemoteHost )
		  or return $subResult;

		if ( !defined($sockedRemoteHost) ) {
			logEvent( 2,
				"Setting DESTINATION_ZABBIX_PROXY does not exist in TA_SETTINGS"
			);
			return $subResult;
...
##
		...
		# {"request":{"cmd":1},"api_version":1}
		$socketRequest = '{"request":{"cmd":1},"api_version":1}' . "\n";

		$socket = IO::Socket::INET->new(
			PeerAddr => $sockedRemoteHost,
			PeerPort => "51101",
			Proto    => "tcp",
		);
		...
		if ( !$socket ) {
			logEvent( 2,
"Error 1 occured in fnFetchProxyConfigData1(): could not connect to host : $@"
			);
			return $subResult;
		...
		print $socket $socketRequest;

		$socket->flush();

		$socketReponse = <$socket>;

		close($socket);
##
		logEvent( 1,
			"Data from remote server $sockedRemoteHost: $socketReponse" )
		  ;
		my $jsonResponseObj1 = from_json($socketReponse);

		if ( $jsonResponseObj1->{response}->{opresult} != 1 ) {

			logEvent( 2,
"Error 3 occured in fnFetchProxyConfigData1(): Remote op result = "
				  . $jsonResponseObj1->{response}->{opresult}
				  . ". Remote error message: "
				  . $jsonResponseObj1->{response}->{error_message} );
			return $subResult;
		}
...
## save to db
		...
		$sth1 = $dbh->prepare("DELETE FROM TA_PROXYDATACACHE_HOSTS");
		$sth1->execute();

		$sth1 = $dbh->prepare("DELETE FROM TA_PROXYDATACACHE_INTERFACES");
		$sth1->execute();

		#

		$sql1 =
"INSERT INTO TA_PROXYDATACACHE_HOSTS (DL_HOSTID,DS_HOST) VALUES (?,?)";
		$sth1 = $dbh->prepare($sql1);

		foreach
		  my $hostRecord ( @{ $jsonResponseObj1->{response}{data}{hosts} } )
		{

			#print $hostRecord->{hostid} . " " . $hostRecord->{host} . "\n";

			$sth1->execute( $hostRecord->{hostid}, $hostRecord->{host} );
		...
		$sql1 =
"INSERT INTO TA_PROXYDATACACHE_INTERFACES (DL_HOSTID,DS_IP) VALUES (?,?)";
		$sth1 = $dbh->prepare($sql1);

		foreach my $interfaceRecord (
			@{ $jsonResponseObj1->{response}{data}{interfaces} } )
		{

		 #print $interfaceRecord->{hostid} . " ". $interfaceRecord->{ip} . "\n";

			$sth1->execute( $interfaceRecord->{hostid},
				$interfaceRecord->{ip} );
...
##
	...
	if ($@) {
		...
		logEvent( 2, "Error 2 occured in fnFetchProxyConfigData1(): $@" );
		return $subResult;
...
CREATE TABLE TA_RECEIVED_TRAP_MSGS ( 
	...
	DS_ZABBIX_SENDER_FORWARDING_PROXY					TEXT,
...
CREATE TABLE TA_PROXYDATACACHE_HOSTS (
    DT_TIMESTAMP	DATETIME    NOT NULL
                    	          DEFAULT (datetime('now','localtime')),
    DL_HOSTID				INTEGER     PRIMARY KEY,                                		   		
    DS_HOST					TEXT		NOT NULL
    									COLLATE 'NOCASE'
);
...
CREATE INDEX idx_TA_PROXYDATACACHE_HOSTS1 ON TA_PROXYDATACACHE_HOSTS ( 
    DS_HOST
);
...
CREATE TABLE TA_PROXYDATACACHE_INTERFACES (
    DT_TIMESTAMP	DATETIME    NOT NULL
                    	          DEFAULT (datetime('now','localtime')),
    DL_HOSTID				INTEGER     PRIMARY KEY,                                		   		
    DS_IP					TEXT		NOT NULL
...
CREATE INDEX idx_TA_PROXYDATACACHE_INTERFACES1 ON TA_PROXYDATACACHE_INTERFACES ( 
    DS_IP
);
...
INSERT INTO [TA_SETTINGS] ([DS_KEY], [DS_VALUE], [DS_DESCRIPTION]) VALUES ('DESTINATION_ZABBIX_PROXY', 'localhost', 'Specifies Zabbix 3.0 proxy this SNMP Trapper forwards Zabbix traps to');
