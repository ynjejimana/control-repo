...
global $zabbixProxyDatabaseFileName;
global $thisFilename;
...
echo getGenericPageHeader1("<b>List of Zabbix hostnames known to this proxy</b> (this info is read from $zabbixProxyDatabaseFileName and used by trapper scripts to identify trap addressee Zabbix hostname by IP address) :");
...
$db = new SQLite3($zabbixProxyDatabaseFileName,SQLITE3_OPEN_READONLY);
$db->busyTimeout($sqliteBusyTimeout);
...
$sql1 = "SELECT hostid,ip,host FROM hosts ORDER BY hostid;";
...
SELECT A.hostid,B.ip,A.host 
FROM hosts A,interface B 
WHERE A.hostid=B.hostid AND B.main=1
ORDER BY A.hostid;
...
return "<table border=0><tr><td width=100%><b>[SNMP trapper V2 Zabbix Proxy ".$thisProxyZabbixHostName."] - </b> $pageTitle</td><td><a target=_blank href='https://wiki.lab.com.au/display/ENG/Monitoring+SNMP+Trap+Messages+V2'>Help</a></td></tr></table><br>";
}
...
echo " AND Forwarding Zabbix Host Identified IS <input type='text' name='frm5text6SearchConditionIsZabbixHostRecognized' id='frm5text6SearchConditionIsZabbixHostRecognized' value='".$tmpStr6."' size=5 >";
...
echo "</b>Forwarding Zabbix Host Identified by IP(1 = yes): </b>".$row[8]."<br>";
...
echo "</b>Forwarding Operation Error Description: </b>".$row[6]."<br>";
