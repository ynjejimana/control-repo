#!/usr/bin/perl
#### Semi-automated Zabbix Proxy core functionality installer (for Zabbix Proxy Internal Monitoring V1)
#### Version 1.36
...
our $coreScriptsDestinationDir = "/srv/zabbix/scripts/zpimonitoring";
...
my $installerLockFile =
  $installerLocksDir . "/08a4c8f5-5e8f-475d-a5f0-a76722b36780";
...
## install zproxyinternalmonitoringcommonutils.pl

	$tmpFileName = 'zproxyinternalmonitoringcommonutils.pl';
...
## install zproxyinternalmonitoringcommonutils.pl
	...
	$tmpFileName = 'ZabbixItemRetrievalLatencyBenchmark.pl';
...
## install cronjob config file zproxy-internal-monitoring

	$tmpFileName = 'zproxy-internal-monitoring';

	installerCopyFile(
		1,
		$installerTempDir . '/' . $tmpFileName,
		'/etc/cron.d/' . $tmpFileName
	);
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}
...
## install diag-n-maintain.pl
	...
	installerFileChmod( $coreScriptsDestinationDir . '/' . $tmpFileName, 0775 );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}
...
## install file installer.spec - just for future debugging and reference reason
