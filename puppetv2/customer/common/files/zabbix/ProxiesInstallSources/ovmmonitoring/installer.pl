#!/usr/bin/perl
#### Semi-automated Zabbix Proxy core functionality installer (for OVM V2 Proxy)
#### Version 1.33
...
our $coreScriptsDestinationDir = "/srv/zabbix/scripts/ovmmonitoring";
...
my $installerLockFile =
  $installerLocksDir . "/7d8fe7c0-ae9a-4225-8e91-5ed2cd3350f7";
...
## - create cluster config directory
	installerMakePath( 1, $coreScriptsDestinationDir . '/clusters', 0 );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}

## install get-hypervisor-stats.pl

	$tmpFileName = 'get-hypervisor-stats.pl';
	...
	installerModifyTextFile( 1, $coreScriptsDestinationDir . '/' . $tmpFileName,
		1, '^\s*my\s*\$loggingLevel\s*=', 'my $loggingLevel = 1' . "\n" );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}

	installerModifyTextFile( 1, $coreScriptsDestinationDir . '/' . $tmpFileName,
		1, '^\s*my\s*\$logOutput\s*=', 'my $logOutput = 2' . "\n" );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}
...
## install diag-n-maintain.pl
	...
	installerFileChmod( $coreScriptsDestinationDir . '/' . $tmpFileName, 0774 );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}

## - create core scripts log file

	$tmpFileName = 'log.txt';

	installerTouchFile( 1, $coreScriptsDestinationDir . '/' . $tmpFileName,
		"" );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}

## copy file xenmonv2.py - will have to be deployed manually on each monitored hypervisor

	$tmpFileName = 'xenmonv2.py';
...
## copy file cluster-example.ini

	$tmpFileName = 'cluster-example.ini';

	installerCopyFile(
		1,
		$installerTempDir . '/' . $tmpFileName,
		$coreScriptsDestinationDir . '/clusters/' . $tmpFileName
	);
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}

## - create core scripts database
...
## - set all permissions to files in the script directory
	...
	my ( $execSubResult, $execResultCode, $execStdErr, @execOutput ) =
	  installerExecuteShellCommand(
		'chmod 774 ' . $coreScriptsDestinationDir . "/xenmonv2.py" );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}

	# - set permissions to the script/clusters directory

	my ( $execSubResult, $execResultCode, $execStdErr, @execOutput ) =
	  installerExecuteShellCommand(
		'chmod -R 775 ' . $coreScriptsDestinationDir . '/clusters' );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}

## - set all permissions to files in the script/clusters directory

	my ( $execSubResult, $execResultCode, $execStdErr, @execOutput ) =
	  installerExecuteShellCommand(
		'chmod -R 664 ' . $coreScriptsDestinationDir . "/clusters/*" );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}
...
"This is an Installation Lock File, preventing multiple successful installer runs. Associated installer: "
		  . $thisScriptFileName );
