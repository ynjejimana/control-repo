#!/usr/bin/perl
#### Semi-automated Zabbix Proxy core functionality installer (for ESXi Monitoring V1 Proxy)
#### Version 1.34
...
our $coreScriptsDestinationDir = "/srv/zabbix/scripts/esximonitoring";
our $zabbixAgentConfigDir      = "/etc/zabbix/zabbix_agentd";
...
my $installerLockFile =
  $installerLocksDir . "/335745f3-24d7-4db9-938d-f3aa6c8d7cac";
...
## - install VMware Perl API SDK

	# unpack installer archive
	my ( $execSubResult, $execResultCode, $execStdErr, @execOutput ) =
	  installerExecuteShellCommand(
		'tar zxf ' . $installerTempDir . '/vmware-vsphere-cli-distrib.tar.gz' );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}

	# make installer script file executable
	my ( $execSubResult, $execResultCode, $execStdErr, @execOutput ) =
	  installerExecuteShellCommand( 'chmod 774 '
		  . $installerTempDir
	...
	# run installer script file
	my ( $execSubResult, $execResultCode, $execStdErr, @execOutput ) =
	  installerExecuteShellCommand( $installerTempDir
		  . '/vmware-vsphere-cli-distrib/vmware-install.pl --default EULA_AGREED=yes'
	  );
	if ( $encounteredErrors > 0 ) {
		return $subResult;
	}
...
## install monitoring script
	...
	$tmpFileName = 'zabbix_perl_api_vmware.pl';
...
## install monitoring script
	...
	$tmpFileName = 'zabbix_dell_wbem_vmware.pl';
...
## install monitoring script
	...
	$tmpFileName = 'zabbix_hp_wbem_vmware5.0.pl';
...
## install monitoring script
	...
	$tmpFileName = 'zabbix_hp_wbem_vmware5.1to5.5.pl';
...
## install monitoring script
	...
	$tmpFileName = 'zabbix_oracle_sunfire_wbem_vmware.pl';
