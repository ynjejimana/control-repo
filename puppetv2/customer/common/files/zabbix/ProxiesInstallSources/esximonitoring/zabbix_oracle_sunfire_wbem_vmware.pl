#! /usr/bin/perl -w
### Monitoring Oracle SunFire Hardware with ESXi 5 (WBEM)
#### Version 1.11
#### Modifications: Premysl Botek (premysl.botek@lab.com)
##################################################################################
...
my %WBEM = (
	...
	"CIM_Processor"            => [ "ElementName", "OperationalStatus" ],
	"CIM_Memory"               => [ "ElementName", "OperationalStatus" ],
	"CIM_PowerSupply"          => [ "ElementName", "HealthState" ],
	"VMware_RollupHealthState" => [ "Name",        "HealthState" ],
	"OMC_Chassis" =>
	  [ "ElementName", "OperationalStatus", "SerialNumber", "Model" ],
);
...
my %REPLASE = (
	...
	"OMC_Chassis" => {
		"OperationalStatus" => "Chassis_OperationalStatus",
		"SerialNumber"      => "Chassis_SerialNumber",
		"Model"             => "Chassis_Model"
	},
	"VMware_RollupHealthState" => {
		"HealthState" => "VMwareRollupHealthState_HealthState",
	},
		...
		if (/^$/) {
			foreach $i ( keys %hardware ) {
				if ( defined $REPLASE{ $args{WBEM_CLASS} }{$i} ) {
					$RESULT{ $REPLASE{ $args{WBEM_CLASS} }{$i} . "_" . $index }
					  = $hardware{$i};
					$index++;
				}
				else {
