...
$fwtest1 = c:\windows\system32\netsh.exe advfirewall firewall show rule name="Allow WinRM HTTPS"
$fwtest2 = c:\windows\system32\netsh.exe advfirewall firewall show rule name="Allow WinRM HTTPS" profile=any
If ($fwtest1.count -lt 5)
{
    Write-Verbose "Adding firewall rule to allow WinRM HTTPS."
    c:\windows\system32\netsh.exe advfirewall firewall add rule profile=any name="Allow WinRM HTTPS" dir=in localport=5986 protocol=TCP action=allow
}
ElseIf (($fwtest1.count -ge 5) -and ($fwtest2.count -lt 5))
{
    Write-Verbose "Updating firewall rule to allow WinRM HTTPS for any profile."
    c:\windows\system32\netsh.exe advfirewall firewall set rule name="Allow WinRM HTTPS" new profile=any
}
Else
{
